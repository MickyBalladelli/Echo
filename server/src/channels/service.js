...
import { notifyChannelInvite, notifyChannelJoin } from '../notifications/service.js'
...
async function getChannelRow(viewerId, slug, transaction, { requireMember = false } = {}) {
  const rows = await sequelize.query(`
    ...
    GROUP BY c.id, owner.id, owner_profile.user_id, viewer_membership.channel_id,
      viewer_membership.user_id, viewer_membership.role
    LIMIT 1
  `, {
...
export async function getChannel(viewerId, slug) {
  return mapChannel(await getChannelRow(viewerId, slug))
}
...
export async function createChannel(ownerId, input) {
  ...
  try {
    await withTransaction(async transaction => {
      const rows = await sequelize.query(`
        INSERT INTO channels (owner_id, name, slug, description, image_url, visibility)
        VALUES (:ownerId, :name, :slug, :description, :imageUrl, :visibility)
        RETURNING id
      `, {
        replacements: { ownerId, slug, ...input, imageUrl: input.imageUrl || null },
        type: QueryTypes.SELECT,
        transaction
      })
  ...
  try {
    await sequelize.query(`
      UPDATE channels SET
        ...
        visibility = COALESCE(:visibility, visibility), updated_at = CURRENT_TIMESTAMP
      WHERE id = :channelId
    `, {
      replacements: {
