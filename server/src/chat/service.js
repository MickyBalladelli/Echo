...
export async function createConversation(userId, input) {
  return withTransaction(async transaction => {
    ...
    if (input.kind === 'direct' && await hasBlock(userId, users[0].id, transaction)) {
...
export async function getConversation(userId, conversationId, transaction) {
  ...
  `, { replacements: { conversationId }, type: QueryTypes.SELECT, ...(transaction ? { transaction } : {}) })
  ...
  const blocks = other ? await sequelize.query(`
    SELECT 1 FROM user_blocks WHERE blocker_id = :userId AND blocked_id = :otherId LIMIT 1
  `, {
  ...
  return {
    ...
    blockedByViewer: Boolean(blocks[0]),
