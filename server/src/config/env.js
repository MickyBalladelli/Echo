...
const envSchema = z.object({
  ...
  CACHE_URL: z.string().trim().regex(/^yasds?:\\/\\//, 'must use yasd:// or yasds://').optional(),
  CACHE_MAX_ENTRIES: z.coerce.number().int().min(1).max(1000000).default(10000),
  CACHE_TTL_MS_FEED: z.coerce.number().int().min(0).max(86400000).default(15000),
  CACHE_TTL_MS_CHANNELS: z.coerce.number().int().min(0).max(86400000).default(30000),
  CACHE_TTL_MS_POPULAR: z.coerce.number().int().min(0).max(86400000).default(60000),
  CACHE_TTL_MS_UNREAD: z.coerce.number().int().min(0).max(86400000).default(30000),
...
export const env = Object.freeze({
  ...
  cacheUrl: result.data.CACHE_URL,
  cacheMaxEntries: result.data.CACHE_MAX_ENTRIES,
  cacheTtlMsFeed: result.data.CACHE_TTL_MS_FEED,
  cacheTtlMsChannels: result.data.CACHE_TTL_MS_CHANNELS,
  cacheTtlMsPopular: result.data.CACHE_TTL_MS_POPULAR,
  cacheTtlMsUnread: result.data.CACHE_TTL_MS_UNREAD,
