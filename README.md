...
The first account created becomes an admin. On an existing database with no admin, the migration promotes the oldest account. Admins can open `/admin` to change user roles, suspend or reactivate accounts, and grant or revoke Verified, Government, and Business badges. Developers sit above admins; only another developer can remove developer access.
