...
- `GET /api/health` checks the process and database readiness (PostgreSQL, local SQLite, or Turso, per `DB_DIALECT`).
- Database queries have a bounded statement timeout. Tune `DB_QUERY_TIMEOUT_MS` only after checking slow-query logs.
